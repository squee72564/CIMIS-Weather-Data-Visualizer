...
def get_weather():
    ...
    db = get_db()
    station_ids = [station['stn_id'] for station in db.execute('SELECT DISTINCT stn_id FROM weather_data').fetchall()]

    exclude_vals = ['stn_id', 'stn_name', 'cimiss_region', 'date', 'jul']
    col_vals = [col[0] for col in db.execute('SELECT * FROM weather_data').description if col[0] not in exclude_vals]
